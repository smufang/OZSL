    ...
    def prepare_data(self):
        ...
        if self.data_config['use_orginal_attr']:
            ...
            if self.data_config['use_der']:
                print('Using DER:')
                seen_data_path = os.path.join(self.data_config['syn_data_root'], "{}_seen_data.pth".format(self.data_config['dataset_name']))
                ...
                self.seen_data_attr = attr_data['seen_attr'][seen_data_label]
                assert len(np.unique(seen_data_label)) == 40 and np.max(seen_data_label) == 39
