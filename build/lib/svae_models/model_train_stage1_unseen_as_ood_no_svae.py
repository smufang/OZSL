...
class Model_train(object):
    ...
    def training(self, checkpoint = -1, checkpoint_num=None, save_path1=None):
        ...
        for epoch in range(checkpoint+1, self.epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_loader):                      
                ...
                z1 = m1 #self.reparameterize(m1, s1) #########!!!!!!!!!
                m2, s2 = self.attr_encoder(input_attr)
                z2 = m2 #self.reparameterize(m2, s2) ###########!!!!!!!!
                
                
                #########!!!!!!!!!
                z_x = z1 #z1.rsample()
                z_attr = z2 #z2.rsample()
                ...
                total_loss =  cls_loss* 1.0 #recon_loss *1.0 + KL_loss * 0.1  + attr_loss *1.0 + cls_loss* 1.0  
                ...
                if (step + 1) % 50 == 0:
                    ...
                    print("Epoch: [%d/%d], Step: [%d/%d], cls_Loss: %.4f, k1: %.4f, k2: %.4f, u: %.4f" %
                         (epoch, self.epoch, step , len(self.train_loader), cls_loss.data.item(), s1.mean().data.item(), s2.mean().data.item(), torch.dot(z_x[1,:], z_attr.squeeze()[1,:]).data.item()))
    ...
    def testing_split_ood_synthetic_vis(self, dataset_config, epoch, seen_classes, known_unseen_classes, dataset = None, threshold = 0.99):
        all_class = dataset_config["seen_classes"] + dataset_config["unseen_classes"]
        all_seen_class = dataset_config["seen_classes"]
        all_unseen_class = dataset_config["unseen_classes"]
        ...
        #input_data = normalize(input_data)
        label_feat = np.array(unseen_label)
        label_feat[label_feat < all_class] = 0
        label_feat[label_feat == all_class] = -1
        # print('len: 41 ', len(input_data[label_feat == 41]))
        tsne_plot(input_data, label_feat, 'svae_stage_2')
    ...
    def testing_split_ood_synthetic_seed(self, dataset_config, epoch, seen_classes, known_unseen_classes, dataset = None, threshold = 0.99):
        ...
        checkpoint1 = torch.load('/home/sethupathy/openworl_zsl/A-Boundary-Based-Out-of-Distribution-Classifier-for-Generalized-Zero-Shot-Learning-master/data_split_no_svae_seed/{}_data_seen_seed{}.pt'.format(dataset_config["dataset_name"], self.seed))
        seen_feat1 = checkpoint1['seen_feat']
        seen_label1 = checkpoint1['seen_label_gt']
        unseen_feat1 = checkpoint1['unseen_feat_split']
        unseen_label1 = checkpoint1['unseen_label_gt']

        checkpoint2 = torch.load('/home/sethupathy/openworl_zsl/A-Boundary-Based-Out-of-Distribution-Classifier-for-Generalized-Zero-Shot-Learning-master/data_split_no_svae_seed/{}_data_unseen_seed{}.pt'.format(dataset_config["dataset_name"], self.seed))
        seen_feat2 = checkpoint2['seen_feat']
        seen_label2 = checkpoint2['seen_label_gt']
        unseen_feat2 = checkpoint2['unseen_feat_split']
        unseen_label2 = checkpoint2['unseen_label_gt']
    ...
    def testing_split_ood_synthetic_seed_conf(self, dataset_config, epoch, seen_classes, known_unseen_classes, dataset = None, threshold = 0.99):
        all_class = dataset_config["seen_classes"] + dataset_config["unseen_classes"]
        all_seen_class = dataset_config["seen_classes"]
        all_unseen_class = dataset_config["unseen_classes"]
        ...
        checkpoint1 = torch.load('/home/sethupathy/openworl_zsl/A-Boundary-Based-Out-of-Distribution-Classifier-for-Generalized-Zero-Shot-Learning-master/data_split_seed/{}_data_seen_seed{}.pt'.format(dataset_config["dataset_name"], self.seed))
        seen_feat1 = checkpoint1['seen_feat']
        seen_label1 = checkpoint1['seen_label_gt']
        unseen_feat1 = checkpoint1['unseen_feat_split']
        unseen_label1 = checkpoint1['unseen_label_gt']

        checkpoint2 = torch.load('/home/sethupathy/openworl_zsl/A-Boundary-Based-Out-of-Distribution-Classifier-for-Generalized-Zero-Shot-Learning-master/data_split_seed/{}_data_unseen_seed{}.pt'.format(dataset_config["dataset_name"], self.seed))
        seen_feat2 = checkpoint2['seen_feat']
        seen_label2 = checkpoint2['seen_label_gt']
        unseen_feat2 = checkpoint2['unseen_feat_split']
        unseen_label2 = checkpoint2['unseen_label_gt']
        ...
        svae_pred_list = [0 for i in range(len(seen_label))] + [2 for i in range(len(unseen_label))]
        svae_correct_list = []
        for i in gt_label:
            if i < all_seen_class:
                svae_correct_list.append(0)
            elif i >= all_seen_class and i < all_class:
                svae_correct_list.append(1)
            else:
                svae_correct_list.append(2)

        confusion_mat_rate(svae_pred_list, svae_correct_list)
        
        print('************Ours*******************')
        our_pred_list = [0 for i in range(len(seen_label))] + [1 for i in range(len(unseen_label_gt))] + [2 for i in range(len(ood_label_gt))]
        our_gt_list = seen_label.tolist() + unseen_label_gt.tolist() + ood_label_gt.tolist()
        our_correct_list = []
        
        for i in our_gt_list:
            if i < all_seen_class:
                our_correct_list.append(0)
            elif i >= all_seen_class and i < all_class:
                our_correct_list.append(1)
            else:
                our_correct_list.append(2)
        
        assert len(our_gt_list) == len(svae_correct_list)

        confusion_mat_rate(our_pred_list, our_correct_list)
    ...
    def testing_split_ood_synthetic_val2(self, dataset_config, epoch, seen_classes, known_unseen_classes, dataset = None, threshold = 0.99):
        all_class = dataset_config["seen_classes"] + dataset_config["unseen_classes"]
        all_seen_class = dataset_config["seen_classes"]
        all_unseen_class = dataset_config["unseen_classes"]
        ...
        attr_data = torch.load('/home/sethupathy/openworl_zsl/A-Boundary-Based-Out-of-Distribution-Classifier-for-Generalized-Zero-Shot-Learning-master/data_seed/{}_attr_seed{}.pth'.format(self.dataset_name, 5))
        attrs = attr_data['seen_attr'][0:20] 
        print(attrs.shape)
        all_attrs = torch.Tensor(attrs).float().cuda()
        all_anchors = self.attr_encoder(all_attrs)[0]
        unseen_anchors = all_anchors  
        # sys.exit()
        ...
        total_ood = len(seen_train_feat[seen_train_label >= 20])
        total_unseen = len(seen_train_feat[seen_train_label < 20])
        assert total_ood + total_unseen == len(seen_train_feat)
        ood_split = seen_ood_count
        unseen_split = seen_unseen_count
        print("seen_ood_percentage: ", ood_split/total_ood)
        print("seen_unseen_percentage: ", unseen_split/total_unseen)
        # sys.exit()
        result['seen_ood_top1'] = ood_split/total_ood  
        result['seen_unseen_top1'] = unseen_split/total_unseen     
    ...
    def testing_split_ood_synthetic_onego(self, dataset_config, epoch, seen_classes, known_unseen_classes, dataset = None, threshold = 0.99):
        all_class = dataset_config["seen_classes"] + dataset_config["unseen_classes"]
        all_seen_class = dataset_config["seen_classes"]
        all_unseen_class = dataset_config["unseen_classes"]
        ...
        try:
            ...
            unseen_bool = np.logical_and(unseen_label_gt < all_class, unseen_label_gt >= all_seen_class)
            unseen_acc = np.sum(pred[unseen_bool]==unseen_label_gt[unseen_bool])
            print("unseen_acc:", unseen_acc/total_unseen)
            result['unseen_acc'] = unseen_acc/total_unseen
            acc_top1 = top1_accuracy(pred[unseen_bool], unseen_label_gt[unseen_bool], gt_label[np.logical_and(gt_label < all_class, gt_label >= all_seen_class)])
            print('acc_unseen_top1: ', acc_top1)
            result['unseen_top1'] = acc_top1

            
            total_seen = len(gt_label[gt_label < all_seen_class])
            seen_split = len(unseen_label_gt[unseen_label_gt < all_seen_class])
            print("seen_percentage: ", seen_split/total_seen)
            seen_bool = unseen_label_gt < all_seen_class
            seen_acc = np.sum(pred[seen_bool]==unseen_label_gt[seen_bool])
            print("seen_acc:", seen_acc/total_seen)
            result['seen_acc'] = seen_acc/total_seen
            acc_top1 = top1_accuracy(pred[seen_bool], unseen_label_gt[seen_bool], gt_label[gt_label < all_seen_class])
            print('acc_seen_top1: ', acc_top1)
            result['seen_top1'] = acc_top1
        
        except:
            print("seen_acc:", None)
            result['seen_acc'] = None
            print('seen_top1: ', None)
            result['seen_top1'] = None
            print('F1_score_seen: ', None)
            result['seen_F1'] = None

            print("unseen_acc:", None)
            result['unseen_acc'] = None
            print('unseen_top1: ', None)
            result['unseen_top1'] = None
            print('F1_score_unseen: ', None)
            result['unseen_F1'] = None
    ...
    def training_der(self, data_config, checkpoint = -1, checkpoint_num=None, save_path1=None):
        ...
        for epoch in range(checkpoint+1, self.epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_loader):                      
                ...
                DER_loss = F.mse_loss(m1_current, m1_prev) + F.mse_loss(m2_current, m2_prev)
                ...
                total_loss =  cls_loss* 1.0  + DER_loss * data_config['der_weight']
